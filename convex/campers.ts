...
// Get meal attendance history with camper details
export const getMealHistory = query({
  args: {},
  handler: async (ctx) => {
    const campers = await ctx.db.query("campers").collect();
    const camperById = new Map(campers.map((camper) => [camper._id, camper]));
    ...
    return attendance
      ...
      .map((record) => {
        const camper = camperById.get(record.camperId);
        return {
          camperId: record.camperId,
          camperName: camper?.name ?? "Unknown Camper",
          team: camper?.team ?? "",
          mealType: record.mealType ?? "unknown",
